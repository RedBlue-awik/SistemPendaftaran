...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <button type="button"
            ...
            data-bs-toggle="modal" data-bs-target="#jalurModal" onclick="resetForm()">
        ...
        @foreach ($jalurs as $j)
            <div class="card-hover bg-white border border-border rounded-2xl p-6 flex flex-col">
                ...
                <div class="flex gap-2 border-t border-border pt-4 mt-auto">
                    <button onclick="editJalur({{ $j->id }})"
                        class="flex-1 px-3 py-2 bg-green-50 border border-border rounded-lg text-sm text-text-main hover:bg-green-100 transition font-medium flex items-center justify-center gap-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <form method="POST" action="{{ route('admin.jalurs.destroy', $j) }}"
                        onsubmit="return confirm('Yakin ingin menghapus jalur ini?')" class="flex-1">
                        @csrf @method('DELETE')
                        <button type="submit"
                            class="w-full px-3 py-2 bg-red-50 border border-red-100 rounded-lg text-sm text-red-600 hover:bg-red-100 transition font-medium flex items-center justify-center gap-1">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border border-border rounded-2xl shadow-2xl overflow-hidden">
                ...
                <div class="modal-body p-6">
                    <form id="jalurForm" method="POST" action="{{ route('admin.jalurs.store') }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="j_nama" name="nama_jalur" required
                                class="w-full px-4 py-2.5 bg-green-50 border border-border rounded-xl focus:outline-none focus:border-accent transition text-text-main"
                                placeholder="Contoh: Jalur Prestasi Akademik">
                        </div>

                        <div class="mb-3">
                            <label class="block text-sm font-medium text-text-main mb-2">Deskripsi</label>
                            <textarea id="j_deskripsi" name="deskripsi" rows="3"
                                class="w-full px-4 py-2.5 bg-green-50 border border-border rounded-xl focus:outline-none focus:border-accent transition text-text-main resize-none"
                                placeholder="Jelaskan singkat tentang jalur ini..."></textarea>
                        </div>

                        <div class="mb-3">
                            <label class="block text-sm font-medium text-text-main mb-2">Batas Pendaftaran</label>
                            <input type="number" id="j_batas" name="batas_pendaftaran" required
                                class="w-full px-4 py-2.5 bg-green-50 border border-border rounded-xl focus:outline-none focus:border-accent transition text-text-main"
                                placeholder="Jumlah maksimal pendaftar">
                        </div>
                    </form>
                </div>
                ...
                <div class="modal-footer border-t border-border bg-white p-4">
                    <button type="button"
                        class="px-4 py-2 bg-gray-100 border border-gray-200 rounded-xl text-text-main hover:bg-gray-200 transition font-medium"
                        data-bs-dismiss="modal">Batal</button>
                    <button type="button" onclick="submitForm()"
                        class="btn-primary px-6 py-2 rounded-xl text-white font-medium transition duration-300">
                        Simpan
                    </button>
                </div>
            </div>
